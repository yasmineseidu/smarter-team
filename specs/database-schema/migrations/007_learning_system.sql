-- Migration 007: Learning and Improvement System
-- Creates tables for agent learning, knowledge management, and continuous improvement
-- Dependencies: 005_system_tables.sql

-- 1. Knowledge Base Table - Central repository of all agent knowledge
...
-- 2. Response Tracking Table - Track every response and its outcomes
...
-- 3. FAQ Management Table - Dynamic FAQ system with usage tracking
CREATE TABLE IF NOT EXISTS faq_management (
    ...
    related_kb_articles UUID[] DEFAULT '{}', -- References to knowledge_base.id
...
-- 4. Agent Learning Table - Agent mistakes, corrections, and improvements
...
-- 5. Agent Performance Analytics Table - Performance metrics over time
...
-- 6. Correction Approval Workflow Table - Workflow for approving corrections
...
COMMENT ON TABLE knowledge_base IS 'Central repository of all agent knowledge and best practices';
COMMENT ON TABLE response_tracking IS 'Tracks every agent response and its outcomes for learning';
COMMENT ON TABLE faq_management IS 'Dynamic FAQ system that evolves based on usage and feedback';
COMMENT ON TABLE agent_learning IS 'Records agent mistakes, corrections, and improvement insights';
COMMENT ON TABLE agent_performance_analytics IS 'Time-series performance metrics for each agent';
COMMENT ON TABLE correction_approval_workflow IS 'Workflow for approving and implementing agent corrections';
